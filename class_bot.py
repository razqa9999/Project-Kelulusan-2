...
# Translator command using Ollama
def translate_text_ollama(text: str, target_language: str) -> str:
    """Translate text using Ollama AI with automatic source language detection."""
    ollama_url = "http://localhost:11434/api/generate"
    ollama_model = "gpt-oss:120b-cloud"
    
    # Prompt yang dirancang agar hanya mengembalikan hasil terjemahan
    system_prompt = """Anda adalah penerjemah profesional. Tugas Anda:
1. Deteksi otomatis bahasa sumber dari teks yang diberikan
2. Terjemahkan teks HANYA ke bahasa yang diminta
3. PENTING: Hanya keluarkan hasil terjemahan, TANPA penjelasan, TANPA metadata, TANPA kalimat tambahan
4. Jika teks sudah dalam bahasa target, keluarkan teks tersebut apa adanya
5. Pertahankan format dan struktur teks original"""
...
Hasil terjemahan:"""
    ...
    try:
        ...
        return "⚠️ Ollama server tidak berjalan. Silakan jalankan Ollama di http://localhost:11434/"
    except requests.exceptions.Timeout:
        return "⚠️ Ollama server membutuhkan waktu terlalu lama. Coba lagi."
    except Exception as e:
        print(f"[ERROR] Ollama translate error: {type(e).__name__}: {e}")
        return f"Terjadi kesalahan: {str(e)}"

@bot.command(name='translate')
async def translate(ctx, target_language: str, *, text: str):
    """
    Menerjemahkan kata/kalimat ke bahasa target dengan deteksi otomatis bahasa sumber.
    
    Penggunaan: +translate <bahasa_target> <teks>
    
    Contoh bahasa target: Indonesia, Inggris, Jepang, Mandarin, Spanyol, Prancis, Jerman, dst.
    
    Contoh:
    +translate Indonesia "Hello, how are you?"
    +translate Inggris "Halo, apa kabar?"
    +translate Jepang "Selamat pagi"
    """
    # Validasi input
    if not text or len(text.strip()) == 0:
        await ctx.send("❌ Teks tidak boleh kosong! Gunakan format: `+translate <bahasa_target> <teks>`")
        return
    
    if len(text) > 500:
        await ctx.send("❌ Teks terlalu panjang! Maksimal 500 karakter.")
        return
    
    # Beri tahu user bahwa sedang menerjemahkan
    status_msg = await ctx.send(f"⏳ Menerjemahkan ke bahasa {target_language}...")
    
    try:
        # Terjemahkan menggunakan Ollama
        translation = await asyncio.to_thread(translate_text_ollama, text, target_language)
        
        if translation.startswith("⚠️") or translation.startswith("Terjadi"):
            # Error message
            await status_msg.edit(content=translation)
        else:
            # Success - format response dengan embed agar lebih rapi
            embed = discord.Embed(
                title="🌐 Hasil Terjemahan",
                color=discord.Color.blue()
            )
            embed.add_field(name="📝 Teks Original", value=text, inline=False)
            embed.add_field(name=f"🎯 Terjemahan ({target_language})", value=translation, inline=False)
            
            await status_msg.delete()
            await ctx.send(embed=embed)
    except Exception as e:
        print(f"[ERROR] Translation command error: {e}")
...
**Penerjemahan:**
`+translate <bahasa_target> <teks>` - Terjemahkan teks ke bahasa target
Contoh: `+translate Indonesia "Good morning"` atau `+translate Inggris "Selamat pagi"`
